...
标签规则：
- `<to_next/>`：当前页内的下一步分隔符。
- `<pause/>`：停顿标记，可放在任意 reveal 内部。表示此段旁白播完后，系统自动暂停播放，让学生自主观察和消化画面内容，直到学生手动点击继续。适合放在展示复杂图解、动画演示、交互操作等需要学生自主观看的场景中。
- `<on_slide>...</on_slide>`：每个 reveal 中如需在画面展示核心知识（如：大字体金句、核心公式、结构化列表、代码块、对比表格或思维导图等），必须写在此标签内。这段内容极大地决定了画面的可视性与丰富度，请多提供结构化内容或 Markdown 格式的提示代码，避免画面知识贫乏。
...
写作原则：
- **强化画面可视化**：利用 `<on_slide>` 强制指示前端呈现富知识对象（如图解、数据展示、结构图、示例代码等），直接赋予画面可观察的知识密度。
